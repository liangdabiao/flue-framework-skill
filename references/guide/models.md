...
### Built-in provider catalog (selected models)

The following providers and models are built into pi-ai and available without registration. Use `init({ providers: { providerId: { apiKey } } })` to pass credentials from environment variables at runtime.

| Provider ID | Model ID | API Protocol | Notes |
|-------------|----------|-------------|-------|
| `anthropic` | `claude-sonnet-4-6` | anthropic-messages | Flagship Anthropic model |
| `anthropic` | `claude-haiku-4-5` | anthropic-messages | Fast, cost-effective |
| `openai` | `gpt-4.1` | openai-responses | Latest GPT-4.1 |
| `openai` | `gpt-4.1-mini` | openai-responses | Lightweight GPT-4.1 |
| `deepseek` | `deepseek-v4-flash` | openai-completions | Fast DeepSeek reasoning model |
| `deepseek` | `deepseek-v4-pro` | openai-completions | High-quality DeepSeek reasoning |
| `openrouter` | (various) | openai-completions | Gateway to multiple providers |

### Using DeepSeek as a built-in provider

DeepSeek is a built-in provider in pi-ai. Its models use the `openai-completions` API protocol and connect to `https://api.deepseek.com` by default. To use it:
...
DeepSeek also provides an **Anthropic-compatible endpoint** at `https://api.deepseek.com/anthropic`. You can use it through the `anthropic` provider with a custom `baseUrl`:
...
const harness = await init({
  ...
  providers: {
    anthropic: {
      baseUrl: env.ANTHROPIC_BASE_URL || 'https://api.deepseek.com/anthropic',
...
### Configuring providers at runtime with init()

The `init()` function accepts a `providers` option that overrides transport settings for any built-in or registered provider. This is the recommended way to configure API keys and custom endpoints from within agent handlers:
...
Settings in `init({ providers })` are merged with the base provider configuration. The `baseUrl` and `apiKey` override the provider defaults, while `headers` are merged with existing headers.

**⚠️ Important:** `registerProvider()` and `configureProvider()` must be called in `src/app.ts`, NOT in agent files. The Flue build process only extracts the handler function — top-level side-effect calls in agent files are discarded. Use `init({ providers })` to configure providers from within agent handlers.
